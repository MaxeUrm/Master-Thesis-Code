% Newton Method solver for LW1F

function sol = newton_solver_LW1F(ML_bulk, ML_surf, K, fun, N_Omega, N_Gamma, epsilon, tau, delta, sol_n, tol, k, l)
    
    sol = sol_n;
    
    while norm(fun(sol)) > tol
        
        f_deriv = spalloc((2*k*N_Omega + (2*l+1)*N_Gamma), (2*k*N_Omega + (2*l+1)*N_Gamma), 8*(k+l)*(N_Omega + N_Gamma));
        ...
        for i = 1:l
            ...
            v1 = W_plus_surf_2(v1);
            M1 = diag(v1);
            M1 = (tau/delta) * ML_surf .* M1;
            f_deriv((2*k*N_Omega + (i-1+k)*N_Gamma + 1):(2*k*N_Omega + (k+i)*N_Gamma),(2*k*N_Omega + i*N_Gamma +1):(2*k*N_Omega + (i+1)*N_Gamma)) = M1;
            if i > 1
                v0 = sol((2*k*N_Omega + (i-1)*N_Gamma +1):(2*k*N_Omega + i*N_Gamma),1); % xi n+(i-1)/l
                v0 = W_minus_surf_2(v0);
                M0 = diag(v0);
                M0 = (tau/(delta*l)) * ML_surf .* M0;
                f_deriv((2*k*N_Omega + (i-1+k)*N_Gamma + 1):(2*k*N_Omega + (k+i)*N_Gamma),(2*k*N_Omega + (i-1)*N_Gamma +1):(2*k*N_Omega + i*N_Gamma)) = M0;
            end
        end
