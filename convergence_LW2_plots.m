...
del_u = @(x,t) 2 * exp(-10*t);

% And u on Gamma as

u_gamma = @(x,t) exp(-10*t);

% We choose the chemical potential as

 w = @(x,t) exp(-10*t) * (norm(x)^2 -1)^2;
...
L2_err_matrix1 = zeros(6,8);
H1_err_matrix1 = zeros(6,8);
...
for i = 1:6
    for j = 1:8
        ...
        [h, tau, err_L2, err_H1, L2_int, H1_int] = convergence_LW2(h_vec(i),tau_vec(j),u,nr);
        
        L2_err_matrix1(i,j) = sqrt(L2_int);
        H1_err_matrix1(i,j) = sqrt(H1_int);
        h_arr(i) = h;
        tau_arr(j) = tau;
    end
end
...
L2_err_matrix = L2_err_matrix1;
H1_err_matrix = H1_err_matrix1;
...
%loglog(tau_arr,L2_err_matrix(7,:),"-square",'LineWidth',2, 'Markersize', 10)
...
subplot(2,2,4)
loglog(tau_arr,H1_err_matrix(1,:).^(5/3),"-o",'LineWidth',2, 'Markersize', 10)
hold on
...
%loglog(tau_arr,H1_err_matrix(7,:),"-square",'LineWidth',2, 'Markersize', 10)
