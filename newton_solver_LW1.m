...
function sol = newton_solver_LW1(ML_bulk, ML_surf, K, fun, N_Omega, N_Gamma, epsilon, delta, sol_n, tol)
    
    sol = sol_n;
    
    while norm(fun(sol)) > tol
        
        f_deriv = spalloc(2*N_Omega + 3*N_Gamma, 2*N_Omega + 3*N_Gamma, 3*N_Omega*(N_Omega + N_Gamma));
