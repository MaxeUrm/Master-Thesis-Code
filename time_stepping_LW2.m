...
function [sol_alpha,sol_beta,sol_gamma,sol_xi,sol_zeta] = time_stepping_LW2(S_bulk,M_bulk,S_surf,M_surf,Nodes_bulk, Elements_bulk, Nodes_surf, Elements_surf, alpha_0, xi_0, T, N, epsilon, sigma, delta, kappa, nr)
    ...
    K = spalloc((2*N_Omega + 3*N_Gamma), (2*N_Omega + 3*N_Gamma), 8*(N_Omega + N_Gamma));
    ...
    sol_n = ones(2*N_Omega+3*N_Gamma,1);
    ...
    for i = 2:N
        ...
        v_3 = - M_surf * xi_n + tau * 0.5 * S_surf * zeta_n;
        v_4 = delta * kappa * 0.5 * S_surf * xi_n + (1/delta) * (1.5 * M_surf * W_minus_surf(xi_n) - 0.5 * M_surf * W_minus_surf(xi_pn)) - 0.5 * M_surf * zeta_n + epsilon * 0.5 * M_surf * gamma_n;
        v_5 = zeros(N_Gamma,1);
        
        b = [v_1; v_2; v_3; v_4; v_5]; % rhs
        
        % true solution for CH LW2
        %{
        fun = @(x) K * x + b + f_nonlin_LW2(ML_bulk, ML_surf, x, N_Omega, N_Gamma, epsilon, delta, weights, alpha_n, xi_n); % final funtion to solve
        %}
        
        % Inhomogenous rhs for convergence plot tests OG
        %
        rhs1 = @(x,t) -10*exp(-10*t) * norm(x)^2 - 8 * sigma * exp(-10*t) * (2*norm(x)^2 -1);
        v1 = zeros(N_Omega,1);
        ...
        rhs2 = @(x,t) - 4 * epsilon * exp(-10*t) + 1/epsilon * ((exp(-10*t) * norm(x))^3 - exp(-10*t) * norm(x)^2) - exp(-10*t) * (norm(x)^2 -1)^2;
        v2 = zeros(N_Omega,1);
        ...
        rhs4 = @(x,t) (1/delta) * ((exp(-10*t))^3 - exp(-10*t)) + 2 * epsilon * exp(-10*t) - exp(-10*t);
        v4 = zeros(N_Gamma,1);
        ...
        fun = @(x) (K * x) + b - v + f_nonlin_LW2(M_bulk, M_surf, x, N_Omega, N_Gamma, epsilon, delta, weights, alpha_n, xi_n); % final funtion to solve
        %}
        
        % sol_n = fsolve(fun, sol_n);
        
        tol = 0.000001;
        sol_n = newton_solver_LW2(M_bulk, M_surf, K, fun, N_Omega, N_Gamma, epsilon, delta, sol_n, tol, weights, alpha_n, xi_n);
