%% Time_stepping_AC1 test for CH-equation with AC-Type Boundary values
...
%% Compute solution
[sol_alpha,sol_beta,sol_gamma,sol_xi] = time_stepping_AC2(S_bulk,M_bulk,S_surf,M_surf,Nodes_bulk, Elements_bulk, Nodes_surf, Elements_surf, alpha_0, xi_0, T, N, epsilon, sigma, delta, kappa, nr);
...
%% Video file creation
%
video = VideoWriter('CH_AC2_vid','MPEG-4');
...
xlim([0,5])
ylim([-5, 5])
legend('Masse im Inneren','Masse am Rand','FontSize',20)
hold off
...
for i = 1:(N+1)
    ...
    en(i) = en(i) + energy_surf(sol_xi(:,i),delta,kappa,Nodes_surf,Elements_surf,S_surf);
end
