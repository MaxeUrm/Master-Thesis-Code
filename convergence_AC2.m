...
function [h, tau, err_L2, err_H1, L2_int, H1_int] = convergence_AC2(mesh,N,u,nr)
...
for i = 2:(N+1)
    x = u_sol(:,i);
    err_L2(i) = transpose(x) * M_bulk * x;
    L2_int = L2_int + (err_L2(i)+err_L2(i-1)) * tau * 0.5;
    
    err_H1(i) = transpose(x) * S_bulk * x;
    H1_int = H1_int + (err_H1(i)+err_H1(i-1)) * tau * 0.5;
end
