...
function [B,Bdry] = assembly_tracematrix(Nodes_bulk,Bdrynodes_bulk,Nodes_surf,N_Omega,N_bdry,N_Gamma)

    B = spalloc(N_Omega,N_Gamma,N_bdry); % final matrix
    
    N = N_Gamma+N_bdry;
    
    Bdry = zeros(N,6); % collect the boundary nodes in order to intersect intervalls later
    
    for i = 1:N_bdry
        Bdry(i,1:2) = Bdrynodes_bulk(i,:);
        Bdry(i,3) = i;
        Bdry(i,4) = 1; % 0 indicates Node with inner origin
        Bdry(i,5) = 0;
        Bdry(i,6) = atan2(Bdrynodes_bulk(i,2),Bdrynodes_bulk(i,1)); % get angle with zero
    end
    
    for i = 1:N_Gamma
        Bdry(i+N_bdry,1:2) = Nodes_surf(i,:);
        Bdry(i+N_bdry,3) = i+N_bdry;
        Bdry(i+N_bdry,4) = 0;
        Bdry(i+N_bdry,5) = 1; % 1 indicates Node with Gamma origin
        Bdry(i+N_bdry,6) = atan2(Nodes_surf(i,2),Nodes_surf(i,1)); % get angle with x axis (to sort like this only works in certain domains)
    end
    
    Bdry = sortrows(Bdry, 6); % Nodes are sortet counterclockwise on the unified boundary
    
    for i = 1:(N-1) % check for double entries i.e. if Gamma node is a Omega node
        if Bdry(i,6) == Bdry(i+1,6)
            Bdry(i,4) = 1;
            Bdry(i,5) = 1;
            Bdry(i+1,:) = []; % remove double entries
        end
    end
    ...
    for i = 1:N_Omega
        for j = 1:N_Gamma
            ...
            % specially for the big circle
            if norm(Nodes_bulk(i,:))>5-1/N_Omega % B_ij is only nonzero if we have a boundary node
                
                % get position of the i-th Omega node
                k1 = 1; % position of the node
                while ~(Nodes_bulk(i,:) == Bdry(k1,1:2))
                    k1 = k1+1; % search for the Node in bdry;
                end
                
                % get position of the j-th Gamma node
                k2 = 1; % position of the node
                while ~(Nodes_surf(j,:) == Bdry(k2,1:2)) 
                ...
                if k1==k2 % we have the same nodes
                    ...
                    while Bdry(p1,4) == 0 % Gamma origin
                        p1 = p1-1;
                        if p1 == 0
                            p1 = N; % don't get out of array
                        end
                    ...
                    while Bdry(n1,4) == 0 % Gamma origin
                        n1 = n1+1;
                        if n1 == N+1
                            n1 = 1; % don't get out of array
                        end
                    ...
                    while Bdry(p2,5) == 0 % Omega origin
                        p2 = p2-1;
                        if p2 == 0
                            p2 = N; % don't get out of array
                        end
                    ...
                    while Bdry(n2,5) == 0 % Omega origin
                        n2 = n2+1;
                        if n2 == N+1
                            n2 = 1; % don't get out of array
                        end
