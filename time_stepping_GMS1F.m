...
function [sol_alpha,sol_beta,sol_gamma,sol_xi,sol_zeta,sol_eta] = time_stepping_GMS1F(S_bulk,M_bulk,S_surf,M_surf,Nodes_bulk, Elements_bulk, Nodes_surf, Elements_surf, alpha_0, xi_0, T, N, k, l, epsilon, sigma, delta, kappa)
    ...
    B = assembly_B(Nodes_bulk, Nodes_surf);
        ...
        for i = 1:k
            ...
            K(((i-1)*N_Omega +1):(i * N_Omega),(2*k*N_Omega + (2*l+1)*N_Gamma +1):(2*k*N_Omega + 2*(l+1)*N_Gamma)) = -(tau/k) * transpose(B) * M_surf;
        end
        ...
        % sixth equation
        K((2*k*N_Omega + (2*l+1)*N_Gamma +1):(2*k*N_Omega + (2*l+2)*N_Gamma),((2*k-1)*N_Omega +1):(2* k * N_Omega)) = B;
        K((2*k*N_Omega + (2*l+1)*N_Gamma +1):(2*k*N_Omega + (2*l+2)*N_Gamma),(2*k*N_Omega + (2*l)*N_Gamma +1):(2*k*N_Omega + (2*l+1)*N_Gamma)) = -diag(ones(N_Gamma,1));
