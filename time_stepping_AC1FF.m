...
function [sol_alpha,sol_beta,sol_gamma,sol_xi] = time_stepping_AC1FF(S_bulk,M_bulk,Nodes_bulk,Elements_bulk,S_surf,M_surf,B, Nodes_surf, Elements_surf, alpha_0, xi_0, T, N, k, l, epsilon, sigma, delta, kappa);
    ...
    K = spalloc((2*k*N_Omega + (l+1)*N_Gamma), (2*k*N_Omega + (l+1)*N_Gamma), 4*(k+l)*(N_Omega + N_Gamma));
        ...
        % fourth equation
        K((2*k*N_Omega + l*N_Gamma +1):(2*k*N_Omega + (l+1)*N_Gamma),((k-1)*N_Omega +1):(k * N_Omega)) = transpose(B);
        K((2*k*N_Omega + l*N_Gamma +1):(2*k*N_Omega + (l+1)*N_Gamma),(2*k*N_Omega + l*N_Gamma + 1):(2*k*N_Omega + (l+1)*N_Gamma)) = - M_surf;
