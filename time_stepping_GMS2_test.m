%% Time_stepping_GMS1 test for CH-equation with GMS Boundary values
...
[Nodes_bulk,Elements_bulk, stats] = distmesh( fd, fh, 0.1, [-1,-1;1,1] );
...
max_width(Nodes_bulk,Elements_bulk,No_Elements)
...
%% Compute solution
[sol_alpha,sol_beta,sol_gamma,sol_xi,sol_zeta,sol_eta] = time_stepping_GMS2(S_bulk,M_bulk,S_surf,M_surf,Nodes_bulk, Elements_bulk, Nodes_surf, Elements_surf, alpha_0, xi_0, T, N, epsilon, sigma, delta, kappa, nr);

%% Plotting the solution with fill3 / trisurf
 
%{
figure(2)
alpha_0 = sol_alpha(:,6); % after 6 steps...
hold on
...
%% Video file creation
%
video = VideoWriter('CH_GMS2_vid','MPEG-4');
...
xlim([0,5])
ylim([-20,25])
legend('Masse im Inneren','Masse am Rand','Gesamtmasse','FontSize',20,'Location','northeast')
hold off
...
ed_surf = zeros(N-2,1)
for i = 3:(N+1)
    ed_bulk(i) = transpose(sol_alpha(:,i)-sol_alpha(:,i-1)) * M_bulk * (sol_alpha(:,i)-sol_alpha(:,i-1));
    ed_surf(i) = transpose(sol_xi(:,i)-sol_xi(:,i-1)) * M_surf * (sol_xi(:,i)-sol_xi(:,i-1));
end

figure(134)
semilogy(t,ed_bulk,'Color',loyolagreen,'LineWidth',2)
hold on
semilogy(t,ed_surf,'Color',limegreen,'LineWidth',2)
title('Störterme','FontSize',fontSize)
xlabel('Zeit','FontSize',fontSize)
ylabel('Energie','FontSize',fontSize)
xlim([0,5])
legend('Störterm im Inneren','Störterm am Rand' ,'FontSize',20)
hold off
toc
