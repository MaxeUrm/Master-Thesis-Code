%% Finer discretization error improvement plots - AC1FF
...
del_u = @(x,t) 2 * exp(-t) * (1 + x(1)*x(2));

% And u on Gamma as

u_gamma = @(x,t) exp(-t) * (1 + x(1)*x(2));
...
% [h_bm, tau_bm, err_L2_bm, err_H1_bm, L2_int_bm, H1_int_bm] = convergence_AC1(0.03,1000,u);
...
for i = 1:length(l_array)
    [L2_int_bdry, H1_int_bdry, L2_int, H1_int] = convergence_AC1FF(200,l_array(i),u);
...
% dof = 400
for i = 1:length(l_array)
    [L2_int_bdry, H1_int_bdry, L2_int, H1_int] = convergence_AC1FF(400,l_array(i),u);
...
% dof = 800
for i = 1:length(l_array)
    [L2_int_bdry, H1_int_bdry, L2_int, H1_int] = convergence_AC1FF(800,l_array(i),u);
...
for i = 1:length(dof_array)
    [L2_int_bdry, H1_int_bdry, L2_int, H1_int] = convergence_AC1FF(dof_array(i),1,u);
...
% l = 4
for i = 1:length(dof_array)
    [L2_int_bdry, H1_int_bdry, L2_int, H1_int] = convergence_AC1FF(dof_array(i),4,u);
...
% l = 16
for i = 1:length(dof_array)
    [L2_int_bdry, H1_int_bdry, L2_int, H1_int] = convergence_AC1FF(dof_array(i),16,u);
...
legend('$L^2(\Omega)$ Fehler','$L^2(\Gamma)$ Fehler', '$H^1(\Omega)$ Fehler', '$H^1(\Gamma)$ Fehler', 'Location', 'southwest');
...
subplot(3,2,6)
loglog(l_array,L2_dof_matrix(:,1),"-o",'LineWidth',2, 'Markersize', 10)
hold on
loglog(l_array,L2_l_matrix_bdry(:,1),"-o",'LineWidth',2, 'Markersize', 10)
...
semilogy(l_array,L2_l_matrix(:,1),"-o",'LineWidth',2, 'Markersize', 10)
hold on
semilogy(l_array,L2_l_matrix_bdry(:,1),"-o",'LineWidth',2, 'Markersize', 10)

semilogy(l_array,H1_l_matrix(:,1),"-x",'LineWidth',2, 'Markersize', 10)
semilogy(l_array,H1_l_matrix_bdry(:,1),"-x",'LineWidth',2, 'Markersize', 10)


title('Feinere Zeitdiskretisierung in $\Omega$','FontSize',fontSize)
